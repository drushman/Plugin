...
/**
 * @file
 * Contains \Drupal\plugin\Component\Utility\Unicode.
 */
...
EOD;
   ...
   *   - \Drupal\plugin\Component\Utility\Unicode::STATUS_MULTIBYTE
   *     Full unicode support using an extension.
   *   - \Drupal\plugin\Component\Utility\Unicode::STATUS_SINGLEBYTE
   *     Standard PHP (emulated) unicode support.
   *   - \Drupal\plugin\Component\Utility\Unicode::STATUS_ERROR
  ...
  public static function strtoupper($text) {
    if (static::getStatus() == static::STATUS_MULTIBYTE) {
      ...
      $text = preg_replace_callback('/\xC3[\xA0-\xB6\xB8-\xBE]/', '\Drupal\plugin\Component\Utility\Unicode::caseFlip', $text);
      return $text;
    }
  }
  ...
  public static function strtolower($text) {
    if (static::getStatus() == static::STATUS_MULTIBYTE) {
      ...
      // Case flip Latin-1 accented letters.
      $text = preg_replace_callback('/\xC3[\x80-\x96\x98-\x9E]/', '\Drupal\plugin\Component\Utility\Unicode::caseFlip', $text);
      return $text;
    }
  }
   ...
   *
   * All functions designed to filter input should use Drupal\plugin_validate_utf8
